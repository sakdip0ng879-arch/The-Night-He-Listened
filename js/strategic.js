...
TK.map = (function(){
  ...
  const SKINS = {
    ...
    studio: () => ({
      name         : 'studio',
      zhouMinVB    : 'tier',                      /* ★ ผูกกับคลิกที่ town โผล่ ดู zhouMinVB() */
  ...
  /* ★ `studio` ผูกชั้นมณฑลเข้ากับ **คลิกเดียวกับที่ town โผล่** (`TIER_TOWN`)
     ⇒ ทั้งหน้ามีจุดเปลี่ยนแค่สองจุด: คลิก 2 (เมืองมา) · คลิก 5 (หมู่บ้านมา + ชั้นมณฑลไป)
  ...
  const zhouLayerOn = () => skin().zhouMinVB === 'tier'
    ? zoomClicks() < TIER_TOWN
    : vb.w >= skin().zhouMinVB;
  ...
  /* ══ ★★★ ชั้นซูม — นับเป็น "คลิกล้อจากภาพเต็มแผ่น" ไม่ใช่ตัวเลข vb.w ดิบ (2026-09-10) ══
     ...
     → เปลี่ยนหน่วยเป็น **จำนวนคลิกจากภาพเต็มแผ่น** ซึ่งเป็นสิ่งที่ผู้อ่านนับได้จริง
       ...
       | 2–4  | **รายมณฑล** — + เมือง `city` ทั้งหมด + ด่าน/ท่า/ปากหุบ/ภูเขา |
  ...
  function buildPins(){
    for (const id in TK.places){
      ...
      /* ★★ `chk` = พิกัดยังไม่ได้สอบเทียบกับแผ่น — **ต้องเห็นบนแผนที่ ไม่ใช่รู้แค่ในโค้ด**
         (เจ้าของทัก 2026-09-10: *"ทำไมมึงถึงลงไปวางบนแผนที่ได้ อันนี้คือเชคแล้วเหรอ?"*)
         เดิมธงนี้บอกแค่ในการ์ดข้อมูลตอนคลิก กับในรายการค้นหา — คนที่แค่กวาดตาดูแผนที่
         จึงเห็นหมุดสี่จุดที่ *หน้าตาเหมือนของจริงทุกประการ* ทั้งที่ตำแหน่งยังเป็นการเดา
         ⇒ ติดคลาสไว้ที่หมุดเลย แล้วให้ CSS ทำให้มันดูไม่เท่ากับจุดที่ยืนยันแล้ว
         ⚠ เหลือ 4 จุดเพราะอีก 3 จุดที่ไม่มีฉากไหนใช้ถูกถอดออกไปแล้ว (เจ้าของเคาะวันเดียวกัน) */
      const g = mk('g',{class:'pin t-'+p.type+(p.chk ? ' chk' : '')});
      g.dataset.id = id;
