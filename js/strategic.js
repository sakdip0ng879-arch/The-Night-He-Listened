...
TK.map = (function(){
  ...
  /* ══ ★★★ เกณฑ์ของ **ชั้นมณฑล** — เส้นห้ามอยู่บนจอโดยไม่มีชื่อ (ชื่อไม่มีเส้นได้) ══════
     ...
     ★★ **แก้ 2026-09-11 (LOG §5.29) — ผิว studio แยกเป็นสองประตู: เส้นไปก่อน ชื่อไปทีหลัง**
       เจ้าของเลือกทางเลือก B หลังดูภาพเทียบสี่แบบ — *เส้น* หายตั้งแต่คลิก 2 · *ชื่อ* อยู่ถึงคลิก 4
       ⚠ **ไม่ขัดบทเรียนข้างบน** — บทเรียนนั้นกัน *เส้นที่ไม่มีชื่อ* (= ลายพื้นที่ไม่บอกอะไร)
         ส่วน *ชื่อที่ไม่มีเส้น* ยังบอกได้ว่าอยู่มณฑลไหน (และไม่มีแผงไหนบอกชื่อมณฑลแทนได้)
       ⚠ และไม่ใช่เลขสองชุดที่เลื่อนออกจากกันได้แบบ 430/380 — สองประตูอ่าน `TIER_CITY`/`TIER_TOWN`
         ตัวเดียวกับที่ชั้นเมืองใช้ ⇒ ขยับไปพร้อมชั้นเมืองเสมอ (ดู zhouGate ข้างล่าง) */
  ...
  const SKINS = {
    ...
    studio: () => ({
      name         : 'studio',
      zhouMinVB    : 'tier',                      /* ★ ผูกกับชั้นซูม: เส้นถึงคลิก 1 · ชื่อถึงคลิก 4 ดู zhouGate() */
  ...
  /* ★ `studio` ผูกชั้นมณฑลเข้ากับ **คลิกเดียวกับที่ชั้นเมืองเปลี่ยน** — สองประตู (2026-09-11 · LOG §5.29)
     · **เส้น** อยู่ถึงคลิก 1 หายที่คลิก 2 (`TIER_CITY` — คลิกเดียวกับที่ city โผล่)
     · **ชื่อ** อยู่ถึงคลิก 4 หายที่คลิก 5 (`TIER_TOWN` — คลิกเดียวกับที่ town โผล่)
     ⇒ ทั้งหน้ายังมีจุดเปลี่ยนแค่สองจุด: คลิก 2 (เมืองมา + เส้นมณฑลไป) · คลิก 5 (หมู่บ้านมา + ชื่อมณฑลไป)
     ไม่ใช่สามจุดที่ไม่เกี่ยวกัน — และเลขไม่เลื่อนตามขนาดจออีกต่อไป
     ⚠ `classic` ยังใช้เลข vb.w เดิม (`ZHOU_MIN_VB`) เพราะมันไม่ได้ใช้ระบบชั้นซูมเลย */
  /* ★ ตัดสินด้วย **คลิก** ไม่ใช่ความกว้าง เมื่อผิวผูกกับชั้นซูม — ไม่งั้นที่คลิกที่ 5 พอดี
     `vb.w >= เกณฑ์` จะยังจริงอยู่หนึ่งคลิก แล้วชั้นมณฑลจะหายช้ากว่าที่ town โผล่หนึ่งจังหวะ
     (ต่างกันแค่จังหวะเดียว แต่มันคือจุดที่ตารางในเอกสารจะเริ่มโกหก)
     ⚠ `classic` ยังใช้เลข vb.w ตรง ๆ เพราะไม่ได้ผูกกับระบบชั้นซูมเลย */
  const zhouGate = tier => skin().zhouMinVB === 'tier'
    ? zoomClicks() < tier
    : vb.w >= skin().zhouMinVB;           /* classic: ประตูเดียว เส้นกับชื่อมาและไปพร้อมกัน */
  const zhouLineOn = () => zhouGate(TIER_CITY);
  const zhouNameOn = () => zhouGate(TIER_TOWN);
  ...
  /* เมืองยุทธศาสตร์ในภาพรวม: ประตู/คลังของแนวรบหลัก + เมืองเอกหนึ่งเมืองต่อมณฑล
     ⚠ ชุดนี้ทำหน้าที่สองอย่างพร้อมกัน — ถอดตัวไหนออกต้องดูว่ามณฑลนั้นเหลือเมืองไหม

     ★ `chenliu` → `dongjun` (2026-09-10) เจ้าของสั่ง: *"เอาเฉินหลิวออกแล้วเอาเมืองอื่น
       มาใส่แทนเพราะมันติดฉูฉาง เวลาซูมออกมาจะเป็นจุดขาวๆ มันไม่สวย"*
       วัดแล้ว: เฉินหลิว–สวี่ชางห่างกัน **56 หน่วย = 28 px** ที่กล้องปก ซึ่งเป็นคู่ที่ชิด
       ที่สุดในชุดแบบทิ้งห่าง (คู่ถัดไปคือ เทียนสุ่ย–เฉินชาง ที่ 50 px)
       → ตงจวิ้นห่างเพื่อนบ้านใกล้สุด 102 หน่วย (51 px) เท่ากับคู่ที่รับได้อยู่แล้ว
       ★★ และมันเป็นตัวแทนที่ *ถูกกว่าเดิม* ด้วย: **แผ่นพิมพ์ "Dong Jun" ไว้เองในฐานะ
          เมืองเอกของเหยี่ยนโจว** (เป็นจุดอ้างอิงของ `zhou_lines.js` เส้น ji|yan อยู่แล้ว)
       ⚠ เหยี่ยนโจวมีจุดที่ระบุสังกัดแค่สองจุด (เฉินหลิว · ตงจวิ้น) — ถ้าถอดทั้งคู่
         มณฑลนี้จะมีชื่อมณฑลลอยอยู่โดยไม่มีเมืองสักเมือง */
  const overviewCities=new Set(['hanzhong','baidi','tianshui','chencang','xiangyang','jiangling','hefei','wancheng','wuwei','jinyang','fanyang','linzi','dongjun','pengcheng','nanhai','jianning']);
  /* ══ ★★★ ชั้นซูม — นับเป็น "คลิกล้อจากภาพเต็มแผ่น" ไม่ใช่ตัวเลข vb.w ดิบ (2026-09-10) ══
     เจ้าของทัก: *"เหมือนต้องเลื่อน scroll ของเมาส์ไปถึง 9 ครั้ง ถึงจะแสดงรายละเอียดเมือง
      มันเยอะไปไหม · ต้องคุยกันนะว่าซูมแค่ไหนเข้าไปเห็นอะไร ไม่งั้นก็จะแก้ไม่รู้จบ"*

     ★★★ **ทำไมของเดิมถึงคุยกันไม่รู้เรื่อง: เกณฑ์เป็น `vb.w` ซึ่งไม่ใช่หน่วยที่คนรู้สึก**
     `vb.w` ของภาพเต็มแผ่นไม่คงที่ — มันคือ `max(W, H/สัดส่วนจอ)` จึงเปลี่ยนตามขนาดหน้าต่าง
     (จอ 1904×845 ได้ 4394 · จอทรงอื่นได้อีกเลข) ⇒ "vb.w 1100" แปลเป็นจำนวนคลิกไม่ได้
     และเลขเดียวกันให้ประสบการณ์คนละอย่างบนสองจอ

     → เปลี่ยนหน่วยเป็น **จำนวนคลิกจากภาพเต็มแผ่น** ซึ่งเป็นสิ่งที่ผู้อ่านนับได้จริง
       และไม่ขึ้นกับขนาดจอเลย · ตารางข้างล่างคือสัญญากับเจ้าของ (เคาะ 2026-09-10):

       | คลิก | เห็นอะไร |
       |------|----------|
       | 0–1  | **ภาพรวม** — เมืองหลวง 7 + เมืองหลักที่เลือกไว้ 16 + ชื่อ/เส้นมณฑล |
       | 2–4  | **รายมณฑล** — + เมือง `city` ทั้งหมด + ด่าน/ท่า/ปากหุบ/ภูเขา · ชื่อมณฑล (ไม่มีเส้น · 2026-09-11) |
       | 5+   | **รายพื้นที่** — + `town` ทั้งหมด = ครบทุกจุดในข้อมูล |

     ⚠ ล้อเดิมหมุน 1.16 เท่าต่อคลิก ทำให้กว่าจะถึงรายละเอียดต้องหมุน 10 คลิก
       เปลี่ยนเป็น 1.35 (`ZOOM_STEP`) — **ต้องเป็นตัวเดียวกับที่ `wheel` ใช้**
       ไม่งั้นตารางข้างบนโกหกทันที
     ⚠ `classic` ไม่ใช้ชั้นพวกนี้เลย (`zoomTiers:false`) — เกณฑ์ vb.w เดิมของมันไม่ถูกแตะ */
  const ZOOM_STEP = 1.35;
  const fullVB    = () => Math.max(W, H / fitAR());      /* vb.w ของภาพเต็มแผ่น */
  /* คลิกที่หมุนเข้ามาแล้วจากภาพเต็มแผ่น (0 = ถอยสุด) */
  const zoomClicks = () => Math.log(fullVB() / vb.w) / Math.log(ZOOM_STEP);
  const TIER_CITY = 2, TIER_TOWN = 5;                    /* คลิกที่ของแต่ละชั้นเริ่มโผล่ */
  const mapLevel=()=>{ const c = zoomClicks();
    return c < TIER_CITY ? 'overview' : c < TIER_TOWN ? 'region' : 'local'; };
  function placeAtZoom(id,p){
    const level=mapLevel();
    /* ⚠ ลำดับสำคัญ: **"ฉากนี้ชี้มาที่นี่" ต้องมาก่อนบัญชีดำของภาพรวมเสมอ**
       ของเดิมเช็คบัญชีดำก่อน ⇒ ฉากที่ชี้ไปโซ่วชุนตอนกล้องกว้าง ได้หมุดกลมไร้ชื่อ (ฉาก 107)
       ★ ปกยังสะอาดเหมือนเดิม เพราะโหมดปกไม่มี `forceLabels` เลยแล้ว (setMarkers ออกก่อน) */
    ...
    return true;                       /* local — ครบทุกจุดในข้อมูล */
  }
  const SYM_RANK = {
  ...
  function scalePins(mu){
    ...
    for (const c of cand){
      ...
      /* ★★★ **ที่ที่ฉากชี้ ไม่เข้าการตรวจชน — มันต้องได้รูปเสมอ** (เจ้าของสั่ง 2026-09-10:
         *"ฉากไหนยังเหลือหมุดกลมก็เอาออก เพราะเรามีไอคอนเมืองใช้เพื่อ Highlight แทนอยู่แล้ว"*)

         ที่มาของหมุดกลมที่ยังโผล่: `pri:0` ทำให้ที่ของฉากได้ *เรียงก่อน* ก็จริง
         แต่มันยังต้องผ่านการตรวจชนอยู่ → **ที่ของฉากสองที่ที่ทับกันเอง ตัวหลังโดนคัด**
         แล้ว `symBox[id]` ไม่ถูกเขียน · `fitPinsToSymbols` จึงหาไม่เจอ และปล่อย
         `.mk-dot` วงกลมไว้เป็นตัวสำรอง — นั่นคือหมุดกลมที่เจ้าของเห็น
         ⚠ ของจริงที่ทับกันเองมีอยู่: `baidi` กับ `yongan` อยู่พิกัดเดียวกันเป๊ะ (721,976)
           และ `wuxia` ห่างไป 9 หน่วย — สามที่นี้เป็นเมืองของฉากเดียวกันได้
         ⇒ ปล่อยให้ทับกันดีกว่าปล่อยให้กลายเป็นจุดกลมไร้ความหมาย */
      if (c.pri === 0){ taken.push(box); keep.add(c.id); continue; }
  ...
  function applyPinVisibility(){
    for (const id in pinEl){
      ...
      /* ★★★ **จุดกลมที่ไม่มีชื่อ ห้ามโผล่ตอนถอยดูทั้งแผ่นดิน** (เจ้าของทัก 2026-09-10:
         *"เวลาซูมออกมาจะเป็นจุดขาวๆ มันไม่สวย"* — ตัวอย่างคือเฉินหลิวที่ติดสวี่ชาง
         ห่างกัน 56 หน่วย = 28 px บนจอ labeler จึงทิ้งชื่อตัวหนึ่ง เหลือจุดเปล่า)

         ★ ที่แก้ไม่ใช่ถอดเฉินหลิวอย่างเดียว แต่ถอด **ทั้งชนิดของบั๊ก**:
           ระดับ overview จุดกลมต้องมากับชื่อเสมอ ไม่งั้นมันไม่ได้บอกอะไรเลย
           — คนดูปกไม่รู้ว่าจุดนั้นคือเมืองอะไร มันจึงเป็นแค่รอยเปื้อนบนภาพ
         ⚠ ระดับ region/local ยังปล่อยจุดเปล่าได้เหมือนเดิม **โดยตั้งใจ** — ตรงนั้น
           ความหนาแน่นคือสาระ ("เห็นความ Abundant ของแต่ละพื้นที่") และคนซูมเข้ามาแล้ว
           ยังฮอเวอร์อ่านชื่อได้ · ที่ปกฮอเวอร์ไม่ได้เพราะมันคือภาพ ไม่ใช่เครื่องมือ */
      const cityDot=skin().zoomTiers&&artOn&&placeAtZoom(id,TK.places[id])&&
                    ['capital','city','town'].includes(TK.places[id].type)&&
                    (mapLevel()!=='overview'||labelPins.has(id));
  ...
  function buildPins(){
    for (const id in TK.places){
      ...
      /* ⛔ คลาส `chk` บนหมุดถูกถอด 2026-09-11 (LOG §5.28 ①) — เจ้าของไม่เอาไอคอนจาง+ขอบประ
         หมุดที่พิกัดยังไม่สอบเทียบจึงหน้าตาเหมือนหมุดทั่วไป · ธง `chk` ในข้อมูล **ยังมีผล**
         ที่การ์ดตอนคลิก (ui.js) · รายการค้นหา (ui-studio.js) · และตัวตรวจเส้นมณฑล
         (check_zhou / zhou_evidence) ยังตัดจุดพวกนี้ออกจากชุดหลักฐานเหมือนเดิม */
  ...
  function layoutAnnotations(){
    ...
    /* ══ ★★ **รูปสัญลักษณ์ก็เป็นสิ่งกีดขวาง** (เจ้าของทัก 2026-09-06:
       *"สัญลักษณ์มันทับชื่อเมืองบนแผนที่ และบางครั้งมันบังตัวหนังสือ คำบรรยายในแผนที่"*)

       ของเดิม `annBlocks` มีแต่ของ *ฉาก* (วงฮาโล · หัวลูกศร · วงปะทะ) — รูปเมือง/ค่าย/ด่าน
       ที่วาดอยู่ตลอดไม่เคยถูกนับเป็นสิ่งกีดขวางเลย ป้ายจึงลงไปนั่งทับรูปได้เต็ม ๆ
       ★ `symBox` มีกล่องจริงของทุกรูปที่ *กำลังแสดงอยู่* อยู่แล้ว (scalePins เขียนไว้)
         เอามาต่อท้าย boxes ตรงนี้ ป้ายก็จะหลบรูปด้วยกลไกเดิมทั้งชุด ไม่ต้องเพิ่มอะไรอีก
       ⚠ symBox เป็น *หน่วยแผนที่* ต้องแปลงเป็นพิกเซลจอเหมือน annBlocks */
    for (const id in symBox){
      const s = symBox[id];
      const [sx, sy] = toPx(s.x, s.y);
      boxes.push({ x:sx, y:sy, w:s.w/mu, h:s.h/mu });
    }
  ...
  function fitPinsToSymbols(){
    clearHot();
    /* ★★★ **โหมดปกไม่มีอะไรร้อน** (เจ้าของสั่ง 2026-09-10: *"อันนี้คือภาพปกเฉยๆ
       ไม่ต้องมี Highlight อะไร เพราะตอนนี้เหมือนมัน Highlight เฉิงตู"*)

       ★ ที่มาของอาการ: `showGallery()` ใน ui-studio.js เรียก `TK.engine.goTo(0,'init')`
       เพื่อรีเซ็ตสถานะ → ฉากที่ 0 ชี้ไปที่เฉิงตู → หมุดเฉิงตูติด `hot` ตามกลไกปกติ
       **ไม่ใช่บั๊กของ hot** แต่เป็นเรื่องที่ปก *ไม่ควรมีฉาก* ตั้งแต่แรก
       ⇒ ตัดที่นี่จุดเดียว ไม่ไปยุ่งกับ goTo เพราะปกยัง**ต้อง**โหลดฉาก 0 ไว้ให้ผู้อ่าน
         กด "เริ่มอ่านเรื่อง" แล้วเดินต่อได้ทันที */
  ...
  function setMarkers(markers, animate, delta){
    ...
    forceLabels = new Set();
    quietLabels = new Set();
    /* ★★★ **โหมดปกไม่รับสถานะของฉากเลย** (เจ้าของสั่ง 2026-09-10: *"อันนี้คือภาพปก
       เฉยๆ ไม่ต้องมี Highlight อะไร"*)

       ที่มาของอาการ: `showGallery()` เรียก `goTo(0,'init')` เพื่อรีเซ็ตสถานะ ฉากที่ 0
       จึงปักหมุดที่ **เฉิงตู กับ อี๋หลิง** แล้วสถานะสามอย่างของฉากรั่วขึ้นปกพร้อมกัน:
         · `hot`   → ออร่าที่เจ้าของเห็นว่า "เหมือนมัน Highlight เฉิงตู"
         · `quiet` → ★★ **ชื่อ "เฉิงตู" หายไปจากปก** เพราะกติกาคือ *ป้ายของฉากพูดแทน
                     ชื่อเมืองแล้ว* — แต่บนปกไม่มีป้ายของฉากให้พูด ชื่อจึงหายเปล่า ๆ
         · `force` → **อี๋หลิงถูกดันขึ้นปก** ทั้งที่ `placeAtZoom` คัดมันออกจากภาพรวมไว้แล้ว
       ⇒ ทั้งสามอาการมีต้นเหตุเดียว · ตัดที่ต้นทางจุดเดียวพอ ไม่ต้องไล่ปะทีละอาการ
       ⚠ ยังต้องเรียก `clearMarkers()` ข้างบนตามปกติ — ปกต้องสะอาดจากของฉากก่อนหน้าด้วย
       ⚠⚠ และต้อง `flushOwners()` ก่อนออก **เหมือนทางออก `if (!markers)` ข้างล่าง** —
         สีเจ้าของแผ่นดินถูกพักไว้รอลูกศรลงจอด ถ้าออกเฉย ๆ ปกจะได้แผ่นดินไม่มีสีฝ่าย */
    if (galleryMode()){ flushOwners(); return; }
    pendingAnn = []; annBlocks = []; annPaths = []; leaderG = null;
    ...
    markers.forEach((m, idx) => {
      ...
      if (m.type === 'pin'){
        ...
        if (m.label && (!labelOnlyPlace || m.strength)){
          ...
          /* ⛔ เคยมี `quietLabels.add(m.place)` ตรงนี้ ด้วยเหตุผลว่า *"ข้อความของฉาก
             พูดแทนชื่อเมืองแล้ว"* — **ถอดออก 2026-09-10** เจ้าของสั่ง: *"เวลา highlight
             สถานที่ไหน ชื่อเมืองมันหาย ไม่เอาแบบนี้ ใส่มันด้วย"*

             ★★★ เหตุผลเดิม **ผิดตั้งแต่สมมติฐาน**: ป้ายของฉากพูดว่า *เกิดอะไรขึ้นที่นั่น*
             (เช่น "ทัพหลวงออกจากที่นี่ ในเอกภพของ…") มัน **ไม่เคยบอกชื่อเมือง**
             คนอ่านจึงเห็นเหตุการณ์ลอยอยู่บนแผ่นดินโดยไม่รู้ว่ามันคือเมืองอะไร
             — และตรงนี้คือที่ที่ต้อง *รู้ให้แน่ที่สุด* เพราะฉากกำลังชี้มาที่มัน
             ⚠ ไม่กลัวทับกัน: กล่องของ `.mk-chip`/`.mk-cap` อยู่ในรายการ `avoid` ของ
               labeler อยู่แล้ว ชื่อเมืองจะหลบไปเอง (ดูที่ประกอบ `avoid` ก่อนเรียก layout) */
        }
      }
    ...
    /* รูปของฉากนี้ต้องถูกเลือกใหม่ก่อน แล้วค่อยให้หมุดกับป้ายเกาะมัน
       ...
       ⚠⚠ **ต้องล้างแคชก่อน** — `scalePins` เปิดหัวด้วย "ถ้า vb.w เท่าเดิม ไม่ต้องคิดใหม่"
       ซึ่งถูกสำหรับ *ขนาด* แต่ผิดสำหรับ *ชุดที่ฉากชี้* · สองฉากติดกันที่กล้องกว้างเท่ากัน
       (ซึ่งเป็นเรื่องปกติมาก) จะข้ามการคิดใหม่ทั้งก้อน ⇒ `symBox` ยังเป็นของฉากก่อน
       ⇒ ที่ที่ฉากใหม่ชี้ไม่มีกล่องรูป → `fitPinsToSymbols` หาไม่เจอ → **ปล่อยหมุดกลมทิ้งไว้**
       ★ นี่คือต้นเหตุของหมุดกลมที่เจ้าของทัก (เจอที่ฉาก 33 · 107 · 127:
  ...
  function bindPanZoom(){
    ...
    host.addEventListener('wheel', e => {
      ...
      if (camCancel) camCancel();
      const p = toMap(e);
      /* ★ ต้องเป็น `ZOOM_STEP` ตัวเดียวกับที่ตารางชั้นซูมใช้ — ตารางนั้นพูดเป็น "คลิก"
         ถ้าล้อหมุนคนละอัตรากับที่ตารางคิดไว้ สัญญาที่เขียนไว้ก็ไม่จริงทันที
         (เดิม 1.16 ⇒ กว่าจะถึงรายละเอียดต้อง 10 คลิก · เจ้าของนับได้ 9 แล้วทัก) */
      const k = e.deltaY > 0 ? ZOOM_STEP : 1/ZOOM_STEP;
      const maxWidth=skin().coverFit?fullVB():W;
