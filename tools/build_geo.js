...
/* ขอบเขตเริ่มปี221: BFS ต้องไม่ขยายอี้โจวขึ้นไปบนชายขอบหลงโย่ว/เหอซี
   และไม่ขยายลงไปแทนเจียวโจว ทั้งสองใช้แนวอ้างอิงที่ตรวจเมืองแล้วใน zhou_lines
   ห้ามใช้กฎนี้กับเขตที่ยึดในเรื่อง (อู่ตู/หลงซี/เหอซี) — mapDelta ยังทำงานตามเดิม
   ชายขอบตะวันตกที่ตัดออกคงภาพภูมิประเทศ ไม่เหมาว่าเว่ยควบคุมที่สูงทั้งหมด */
require(path.join(ROOT,'data','zhou_lines.js'));
const yiNorth=TK.zhou.edges.find(e=>e.a==='yong'&&e.b==='yi').pts;
const yiSouth=TK.zhou.edges.find(e=>e.a==='yi'&&e.b==='jiao').pts;
function boundaryY(line,x){
...
let northTrim=0,southTransfer=0;
for(let i=0;i<own.length;i++){
  const id=ids[own[i]];
  if(id!=='yizhou'&&id!=='hanzhong')continue;
  const x=(i%W+.5)*CELL,y=(Math.floor(i/W)+.5)*CELL;
  const n=boundaryY(yiNorth,x),s=boundaryY(yiSouth,x);
  if(n!==null&&y<n){own[i]=-1;northTrim++;}
  else if(id==='yizhou'&&s!==null&&y>s){own[i]=ids.indexOf('jiaozhou');southTransfer++;}
}

/* ══ ★★★ ที่ดินที่ตัดออกจากฮั่น **ต้องมีเจ้าของใหม่** ไม่ใช่ปล่อยว่าง ══════════
   เจ้าของทัก 2026-09-10: *"GPT เอาสีออกแต่ไม่ระบายเว่ยทับลงไป ... พื้นที่ที่เปลี่ยน
    ตอนนั้นมันมีผลต่อเนื้อเรื่องในช่วงเปิดเรื่องตอนบุกเหนือ"*

   ★ วัดของจริงก่อนแก้: การตัดข้างบนทำให้ที่ดิน **3,198 ช่อง** (กรอบ x 0–233 · y 277–738)
     กลายเป็นไร้เจ้าของ · ความครอบคลุมแผ่นดินตกจาก **92.1% → 89.3%**
     บนจอมันคือ *แถบซีดพาดขอบตะวันตก* กับ **ขอบตรงเป๊ะแนวดิ่ง** ตรงที่เส้นตัดผ่าน
     และวงเน้นพื้นที่ของฉาก (§17) ก็ลากตามขอบปลอมนั้นไปด้วย — ฉาก c2-06/c3-04 เห็นชัดสุด

   ⚠⚠ **ทำไมปล่อยว่างไม่ได้** — สามข้อ ไม่ใช่ข้อเดียว
     1. `DECISIONS §3`: ปี 221 แผ่นดินมีเจ้าของครบทุกตารางนิ้ว ชั้นสีเขตคลุมบกทั้งผืน
     2. **เนื้อเรื่องพัง**: การบุกเหนือเล่าด้วย *สีที่พลิก* — หลงซี c2-06 · จินเฉิง c3-04 ·
        อู่เว่ย/จางเย่ c3-08 · ที่ดินที่ไม่มีเจ้าของจะไม่พลิกสีตลอดกาล ผู้อ่านจึงเห็นทัพฮั่น
        ยึดได้แค่ริ้วบาง ๆ แทนที่จะเห็นแผ่นดินเปลี่ยนมือ
     3. รูปเขตกลายเป็น **เส้นตรงที่ไม่มีอะไรรองรับ** — ไม่ใช่ชายฝั่ง ไม่ใช่สันเขา

   ★ ทางแก้ที่ *ไม่* ยกเลิกเจตนาเดิมของการตัด: ให้เพื่อนบ้าน **ฝั่งเหนือเส้น** โตเข้ามาแทน
     (จินเฉิง · หลงซี · อู่ตู — เว่ยทั้งชุดตอนเปิดเรื่อง) โดย **ห้ามโตข้ามเส้นยง–อี้**
     ⇒ ตอนเปิดเรื่องแถบนี้เป็นสีเว่ย · พอฮั่นยึดหลงซี/จินเฉิง/อู่ตู มันพลิกเป็นเขียวไปด้วย
     ⇒ ได้ทั้งสองอย่าง: ฮั่นไม่ล้ำหลงโย่ว/เหอซีตั้งแต่ต้น **และ** ไม่มีแผ่นดินไร้เจ้าของ

   ⚠ ห้ามใช้ "กำแพงกั้น" เป็นช่องในตาราง — จะเหลือร่องไร้เจ้าของกว้าง 1 ช่องคาแนวเส้น
     ต้องคุมด้วย *ฝั่งของเส้น* (`side`) แทน ซึ่งไม่มีร่อง */
const side = new Int8Array(W * H);          /* -1 = เหนือเส้น · 1 = ใต้เส้น · 0 = นอกช่วงเส้น */
for (let i = 0; i < own.length; i++){
  const x = (i % W + .5) * CELL, y = (Math.floor(i / W) + .5) * CELL;
  const n = boundaryY(yiNorth, x);
  side[i] = n === null ? 0 : (y < n ? -1 : 1);
}
let refill = 0, f4 = [];
for (let i = 0; i < own.length; i++) if (own[i] >= 0) f4.push(i);
while (f4.length){
  const next = [];
  for (const i of f4){
    const x = i % W, y = (i / W) | 0, k = own[i];
    for (const [dx, dy] of [[1,0],[-1,0],[0,1],[0,-1]]){
      ...
      const j = ny * W + nx;
      if (own[j] >= 0 || !land[j] || outside[j]) continue;
      if (side[j] !== side[i]) continue;      /* ★ โตข้ามเส้นยง–อี้ไม่ได้ ฮั่นจึงไม่ไหลกลับขึ้นไป */
      own[j] = k; refill++; next.push(j);
    }
  }
  f4 = next;
}
console.log(`ขอบเขตปี221: ตัดสีฮั่นล้ำเหนือ ${northTrim} ช่อง · คืนเจียวโจว ${southTransfer} ช่อง` +
            ` · **คืนเจ้าของให้ที่ดินที่ตัด ${refill} ช่อง** (เพื่อนบ้านฝั่งเหนือเส้นโตเข้ามาแทน)`);
  ...
  for (const loop of traceWithNeighbors(k)) {
    ...
    for (const run of runs) {
      ...
      if (j >= 0) {
        ...
        // ขอบกรอบต้องคงมุมฉาก: Chaikin ตัดมุมซ้ายล่างจนเสียพื้นที่บกเป็นลิ่มใหญ่
        const sm = j === -2 ? dp(pts, DPTOL).map(rnd) : (closedRing ? smoothOpen(pts.concat([pts[0]])) : smoothOpen(pts));
