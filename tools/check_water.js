...
/* ★ raster ของ **น้ำเปิด** (ทะเล+ทะเลสาบ) แยกไว้ต่างหาก — ใช้บอกว่าปลายไหนคือ "ปากแม่น้ำ"
   ⚠ ปลายที่จบติดทะเลหรือทะเลสาบ **ไม่ใช่รอยขาด** มันคือที่ที่ลำน้ำจบจริง ๆ
     ถ้าไม่ยกเว้นข้อนี้ ตัวตรวจจะฟ้องปากแม่น้ำทุกปากตลอดไป และตัวเลขจะไม่มีวันเป็นศูนย์
     (วัดแล้ว: ห้าจุดสุดท้ายที่ค้างอยู่ มีน้ำล้อมรอบ 166–224 จาก 225 ตัวอย่าง — ปากแม่น้ำล้วน) */
const openOnly = new Uint8Array(GW * GH);
{
  const save = ours.slice();
  ours.fill(0);
  for (const r of PW.sea)   fillRing(r, 1);
  for (const r of PW.lakes) fillRing(r, 1);
  openOnly.set(ours);
...
for (const r of PW.rivers){
  ...
  for (const e of cand){
    ...
    nEnds++;
    if (nearOpen(e.x, e.y)) continue;             /* ปากแม่น้ำ — จบถูกแล้ว */
    ...
    /* ★★ เกณฑ์: ต้องเจอน้ำที่ยังไม่ได้วาด **ติดกันอย่างน้อยห้าช่วง (>= 15 หน่วย)**
       ตั้งจาก *สิ่งที่ต้องการจับ* ไม่ใช่ตั้งให้ตัวเลขลงศูนย์:
         รอยที่ป้าย/ไอคอนของแผ่นทำให้ลำน้ำขาดจริง **กว้าง 15–65 หน่วย** ทุกอัน
         (ดูรายการสะพานที่ build_plate_water พิมพ์ออกมา — 6 ถึง 65 หน่วย)
       ⚠ เกณฑ์ที่ต่ำกว่านี้ไล่ไม่มีวันจบ และผมพิสูจน์แล้วสองรอบ:
         landmask ย่อภาพมาแบบ bicubic ขอบน้ำของมันจึงเลยขอบหมึกจริงไปราวหนึ่งช่อง
         ทุกปลายที่ยืดจนสุดหมึกจริงแล้วยังโดนฟ้องว่า "มีน้ำอีก 6–9 หน่วย"
         พอยืดตามไป **ก็เกิดปลายใหม่ที่โดนฟ้องอีก ที่พิกัดใหม่** — วนแบบนั้นไม่จบ
       ★ บทเรียน: **ตัวเลขที่ขยับที่ทุกครั้งที่ไล่ ไม่ใช่รายการงาน มันคือหน่วยวัดที่ละเอียดเกินของจริง** */
    if (hit >= 5) cut.push({ x:e.x, y:e.y, ahead:hit });
  }
}
...
const CUT_MAX = 0;    /* ★ ศูนย์ · เจ้าของสั่งไล่ให้หมด 2026-09-05 — ห้ามกลับมาเกินศูนย์อีก */
