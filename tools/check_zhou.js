...
/* ⚠⚠⚠ **จุดที่ยังติดธง `chk:true` ห้ามเข้าการตรวจ** (เจ้าของทัก 2026-09-10:
   *"มันยังมีชื่อเมืองที่รอตรวจแผนที่แล้วใส่เข้ามาด้วย ... ทำไมมึงถึงลงไปวางบนแผนที่ได้
    อันนี้คือเชคแล้วเหรอ?"* — คำตอบคือ **ยังไม่ได้เช็ค**)
   `chk:true` = พิกัดยังไม่ได้สอบเทียบกับแผ่น · เอาจุดแบบนั้นมาตรวจว่า "เส้นแบ่งถูกไหม"
   คือ **เอาของที่ยังไม่รู้ว่าถูก ไปรับรองของอีกอันหนึ่ง** — ตัวเลขจะดูดีขึ้นโดยไม่มีอะไรแน่นขึ้น
   ★ ผมเองเป็นคนพลาดข้อนี้ตอนย้ายตาราง 2026-09-10: ใส่ `province` ให้ `qilian`/`gaoping`
     ทั้งที่สองจุดนั้น `chk:true` ⇒ มันไหลเข้าชุดหลักฐานเงียบ ๆ · ที่นี่คือที่ที่กันไว้ */
const EXPECT = {};
for (const [id, p] of Object.entries(P)) if (p.province && !p.chk) (EXPECT[p.province] ||= []).push(id);
const CHK = Object.keys(P).filter(id => P[id].chk);
...
if (CHK.length)
  console.log('⚠ จุดที่พิกัดยังไม่ได้สอบเทียบกับแผ่น (chk:true) ' + CHK.length + ' จุด — ' +
              '**ไม่ถูกนับเป็นหลักฐาน** แต่ยังถูกวาดบนแผนที่อยู่\n   ' + CHK.join(' '));
